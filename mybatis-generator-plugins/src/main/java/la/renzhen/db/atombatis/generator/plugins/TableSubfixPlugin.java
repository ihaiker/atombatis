...
public class TableSubfixPlugin extends PluginAdapterEnhance {
    ...
    public boolean modelExampleClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        Field tableSubfix = new Field("tableSubfix", PrimitiveTypeWrapper.getStringInstance());
        tableSubfix.setVisibility(JavaVisibility.PRIVATE);
        addField(topLevelClass, introspectedTable, tableSubfix);

        //添加TableSelect继承
        topLevelClass.addSuperInterface(new FullyQualifiedJavaType(TableSubfix.class.getName()));
        topLevelClass.addImportedType(TableSubfix.class.getName());
    ...
    public boolean sqlMapSelectByPrimaryKeyElementGenerated(XmlElement element, IntrospectedTable introspectedTable) {
        ...
        TextElement subSentence = new TextElement("from " + tablePrefix + "_${tableSubfix}");
    ...
    public boolean clientSelectByPrimaryKeyMethodGenerated(Method method, Interface interfaze, IntrospectedTable introspectedTable) {
        if (checkIsIdentity(method, interfaze, introspectedTable)) {
            ...
            Parameter parameter = new Parameter(FullyQualifiedJavaType.getStringInstance(), "tableSubfix");
            parameter.addAnnotation("@Param(\"tableSubfix\")");
     ...
     * @return
     */
    private boolean checkIsIdentity(Method method, Interface interfaze, IntrospectedTable introspectedTable) {
        String slave = method.getParameters().stream().map(Parameter::getName).reduce("", String::concat);
        String tablePrimaryKey = introspectedTable.getAllColumns().stream().filter(IntrospectedColumn::isIdentity)
                .map(IntrospectedColumn::getActualColumnName).reduce("", String::concat);

        //主键和分库策略是相同的COLUMN
        //return slave.equals(tablePrimaryKey);
        return false;
    }
    ...
    private void resetInsertXmlElementTableName(XmlElement element, IntrospectedTable introspectedTable) {
        ...
        data[2] = tablePrefix + "_${tableSubfix}";
    ...
    private void resetUpdateXmlElementTableNameNotMapType(XmlElement element, IntrospectedTable introspectedTable) {
        String tablePrefix = introspectedTable.getTableConfiguration().getTableName();
        List<Element> elements = element.getElements();
        TextElement subSentence = new TextElement("update " + tablePrefix + "_${tableSubfix}");
        elements.set(0, subSentence);
    }
    ...
    private void resetCountByExample(XmlElement element, IntrospectedTable introspectedTable) {
        ...
        String content = elements.get(0).getFormattedContent(0);
        String[] data = content.split(" ");
        data[3] = tablePrefix + "_${tableSubfix}";
        TextElement subSentence = new TextElement(MysqlSplitingTablePlugin.join(" ", data));
        elements.set(0, subSentence);
    }
